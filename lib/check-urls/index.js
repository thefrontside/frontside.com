...
const checkURLs = async () => {
  ...
  if (originalHash) {
    try {
      ...
      console.log("hash verified, pages haven't changed");
    } catch (error) {
      if (isCI)
        console.log(
          '::error file=sitemap.txt::The URLs have changed. Try running `yarn netlify build` locally and commiting the sitemap.txt change.'
        );
      throw new Error(
        `Previous hash of sitemap.txt does not match the updated hash: ${hash}`
      );
    }
  } else {
    if (isCI) console.log(`::set-output name=url-hash::${hash}`);
    console.log(`hash value saved: ${hash}`);
  }
};
